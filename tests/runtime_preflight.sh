...
write_config "${RUNTIME}/valid.yaml" "${RUNTIME}/a.crt" "${RUNTIME}/a.key" 18510
...
write_config "${RUNTIME}/invalid.yaml" "${RUNTIME}/invalid.crt" "${RUNTIME}/a.key" 18511
expect_failure invalid_pem 'certificate PEM parse.*contains no certificates' \
  "${BIN}" --config "${RUNTIME}/invalid.yaml" --check

write_config "${RUNTIME}/mismatch.yaml" "${RUNTIME}/a.crt" "${RUNTIME}/b.key" 18512
...
write_config "${RUNTIME}/broken.yaml" "${RUNTIME}/a.crt" \
  "${RUNTIME}/live/privkey.pem" 18513
...
python3 -c 'import socket,time; s=socket.socket(); s.bind(("127.0.0.1",18515)); s.listen(); time.sleep(30)' &
OCCUPIER_PID=$!
trap 'kill "${OCCUPIER_PID}" 2>/dev/null || true' EXIT
sleep 0.1
write_config "${RUNTIME}/occupied.yaml" "${RUNTIME}/a.crt" "${RUNTIME}/a.key" 18515
"${BIN}" --config "${RUNTIME}/occupied.yaml" --check >"${RUNTIME}/occupied-no-bind.log" 2>&1
expect_failure occupied_bind 'listener bind HTTPS 127.0.0.1:18515.*conflicting address 127.0.0.1:18515' \
