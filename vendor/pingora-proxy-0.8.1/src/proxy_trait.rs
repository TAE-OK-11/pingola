...
#[allow(dead_code)]
#[trait_variant::make(ProxyHttp: Send)]
pub trait LocalProxyHttp {
    ...
    /// Return a precomputed immutable upstream peer without allocating.
    ///
    /// Implementations with a fixed set of peers can override this method to
    /// avoid constructing and boxing an identical [`HttpPeer`] for every
    /// request. Dynamic implementations continue to use [`Self::upstream_peer`].
    fn precomputed_upstream_peer<'a>(&'a self, _ctx: &Self::CTX) -> Option<&'a HttpPeer> {
        None
    }
...
pub async fn default_fail_to_proxy(session: &mut Session, error: &Error) -> FailToProxy {
    ...
    if code > 0 {
        session.respond_error(code).await.unwrap_or_else(|error| {
            error!("failed to send error response to downstream: {error}");
        });
    }
