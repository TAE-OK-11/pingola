...
pub fn current_handle() -> Handle {
    if let Some(pools) = CURRENT_HANDLE.get() {
        ...
        let index = runtime_index(pools.len());
        pools[index].clone()
    } else {
...
/// Pick a runtime from a non-empty pool. The single-worker hot path avoids
/// initializing or consulting thread-local random-number-generator state.
#[inline]
fn runtime_index(threads: usize) -> usize {
    assert!(threads != 0, "runtime pool cannot be empty");
    if threads == 1 {
        0
    } else {
        rand::thread_rng().gen_range(0..threads)
    }
}
...
impl NoStealRuntime {
    ...
    pub fn get_runtime(&self) -> &Handle {
        let index = runtime_index(self.threads);
...
#[test]
fn test_single_worker_runtime_selection_is_stable() {
    for _ in 0..1_000 {
        assert_eq!(runtime_index(1), 0);
    }
}

#[test]
fn test_multi_worker_runtime_selection_stays_in_bounds() {
    for threads in 2..=16 {
        for _ in 0..1_000 {
            assert!(runtime_index(threads) < threads);
        }
    }
}
