...
<<<<<<< vendor/pingora-proxy-0.9.0/src/lib.rs
pub use proxy_trait::{FailToProxy, ProxyHttp, ProxyWarnLogContext};
=======
pub use proxy_trait::{
    default_fail_to_proxy, FailToProxy, ProxyHttp, RawSocketHandle,
};
pub use pingora_cache::{CacheMeta, ForcedFreshness, HitHandler};
>>>>>>> vendor/pingora-proxy-0.8.1/src/lib.rs
...
>>>>>>> vendor/pingora-proxy-0.8.1/src/lib.rs
    ...
    {
        let owned_peer;
        let peer = if let Some(peer) = self.inner.precomputed_upstream_peer(ctx) {
            peer
        } else {
            owned_peer = match self.inner.upstream_peer(session, ctx).await {
                Ok(peer) => peer,
...
/// Create an [`HttpProxy`] with a custom upstream connector.
pub fn http_proxy_custom<SV, C>(
    conf: Arc<ServerConf>,
    inner: SV,
    connector: C,
) -> HttpProxy<SV, C>
where
    ...
    C: custom::Connector,
{
    let mut proxy = HttpProxy::new_custom(inner, conf, connector, None, None);
    proxy.handle_init_modules();
    proxy
}
