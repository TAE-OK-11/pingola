...
TCMALLOC_IMAGE=${TCMALLOC_IMAGE:-pingora:allocator-tcmalloc}
...
docker build --build-arg RUST_TARGET_CPU=x86-64-v2 \
  --build-arg ALLOCATOR=tcmalloc -t "${TCMALLOC_IMAGE}" "${ROOT}"
docker build --build-arg RUST_TARGET_CPU=x86-64-v2 \
  --build-arg ALLOCATOR=jemalloc -t "${JEMALLOC_IMAGE}" "${ROOT}"
docker build --build-arg RUST_TARGET_CPU=x86-64-v2 \
  --build-arg ALLOCATOR=system-allocator -t "${SYSTEM_IMAGE}" "${ROOT}"

set +e
BENCH_PROFILE=smoke BENCH_ROUNDS="${ROUNDS}" PINGORA_IMAGE="${TCMALLOC_IMAGE}" \
  BENCH_OUTPUT="${OUTPUT}/tcmalloc" "${ROOT}/bench/compare.sh"
TCMALLOC_RC=$?
...
for allocator in ("tcmalloc", "jemalloc", "system"):
...
echo "allocator comparison=${OUTPUT} tcmalloc_rc=${TCMALLOC_RC} jemalloc_rc=${JEMALLOC_RC} system_rc=${SYSTEM_RC}"
exit $((TCMALLOC_RC != 0 || JEMALLOC_RC != 0 || SYSTEM_RC != 0))
