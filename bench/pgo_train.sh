...
case "${SCENARIO}" in
  h1|h2|tls|tail) ;;
  *) echo "unsupported PGO scenario: ${SCENARIO}" >&2; exit 2 ;;
esac
...
wait_for_tcp() {
  local port=$1 process_pid=$2 name=$3
  for _ in {1..160}; do
    if (exec 3<>"/dev/tcp/127.0.0.1/${port}") 2>/dev/null; then exec 3>&-; return 0; fi
    if ! kill -0 "${process_pid}" 2>/dev/null; then echo "${name} exited before readiness" >&2; return 1; fi
    sleep 0.05
  done
...
train_h2() {
  ...
  run_h2load h2-normal -n 48000 -c 4 -m 16 -w 16 -W 20 --sni pgo.test \
    -H 'host: pgo.test' -H 'accept-encoding: identity' \
    -H 'authorization: Bearer pgo-training-token' -H 'cookie: session=pgo; preferences=h2' \
    "https://127.0.0.1:${HTTPS_PORT}/json/512"
...
train_tls() {
  ...
  aes256_ok=0; aes128_ok=0; chacha_ok=0
  for _ in $(seq 1 192); do
    if printf 'GET /json/512 HTTP/1.1\r\nHost: pgo.test\r\nConnection: close\r\n\r\n' | \
      timeout 4 openssl s_client -connect "127.0.0.1:${HTTPS_PORT}" -servername pgo.test \
        -tls1_3 -alpn http/1.1 -ciphersuites TLS_AES_256_GCM_SHA384 -quiet >/dev/null 2>&1; then
      aes256_ok=$((aes256_ok + 1))
    fi
    if printf 'GET /json/512 HTTP/1.1\r\nHost: pgo.test\r\nConnection: close\r\n\r\n' | \
      timeout 4 openssl s_client -connect "127.0.0.1:${HTTPS_PORT}" -servername pgo.test \
        -tls1_3 -alpn http/1.1 -ciphersuites TLS_AES_128_GCM_SHA256 -quiet >/dev/null 2>&1; then
      aes128_ok=$((aes128_ok + 1))
    fi
    if printf 'GET /json/512 HTTP/1.1\r\nHost: pgo.test\r\nConnection: close\r\n\r\n' | \
      timeout 4 openssl s_client -connect "127.0.0.1:${HTTPS_PORT}" -servername pgo.test \
        -tls1_3 -alpn http/1.1 -ciphersuites TLS_CHACHA20_POLY1305_SHA256 -quiet >/dev/null 2>&1; then
      chacha_ok=$((chacha_ok + 1))
    fi
  done
  if ((aes256_ok == 0 || aes128_ok == 0 || chacha_ok == 0)); then
    echo "TLS 1.3 cipher training failed: aes256=${aes256_ok} aes128=${aes128_ok} chacha=${chacha_ok}" >&2
    exit 1
  fi
  ...
  { printf 'GET /json/512 HTTP/1.1\r\nHost: pgo.test\r\nConnection: keep-alive\r\n\r\n'; sleep 2; } | \
    timeout 4 openssl s_client -connect "127.0.0.1:${HTTPS_PORT}" -servername pgo.test \
      -tls1_3 -alpn http/1.1 -ign_eof -sess_out "${OUTPUT_DIR}/tls-session.pem" \
      >"${OUTPUT_DIR}/tls-session-new.log" 2>&1
  session_rc=$?
  set -e
  if [[ ${session_rc} -ne 0 && ${session_rc} -ne 124 ]]; then exit 1; fi
  ...
  for _ in $(seq 1 256); do
    ...
    { printf 'GET /json/512 HTTP/1.1\r\nHost: pgo.test\r\nConnection: keep-alive\r\n\r\n'; sleep 0.05; } | \
      timeout 4 openssl s_client -connect "127.0.0.1:${HTTPS_PORT}" -servername pgo.test \
        -tls1_3 -alpn http/1.1 -sess_in "${OUTPUT_DIR}/tls-session.pem" \
        -sess_out "${OUTPUT_DIR}/tls-session-next.pem" \
        >>"${OUTPUT_DIR}/tls-resumption.log" 2>&1
    resume_rc=$?
    set -e
    if [[ ${resume_rc} -ne 0 && ${resume_rc} -ne 124 ]]; then exit 1; fi
...
train_tail() {
  ...
  for pid in "${pause_pids[@]}"; do wait "${pid}"; done
  ...
  "${CLIENT_BIN}" --port "${HTTP_PORT}" --threads 64 --requests-per-thread 250 \
    --path /json/512 --expected-length 512 --body-validation any
  run_h2load tail-h2-burst -n 12000 -c 64 -m 32 -w 15 -W 18 --sni pgo.test \
    -H 'host: pgo.test' -H 'accept-encoding: identity' \
    "https://127.0.0.1:${HTTPS_PORT}/json/512"
}
...
openssl pkey -in "${RUNTIME_DIR}/key.pem" -check -noout >>"${OUTPUT_DIR}/openssl-key.log" 2>&1
