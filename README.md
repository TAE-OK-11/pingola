...
Rust toolchain은 1.98.1이며 Cargo lockfile은 직접 의존성의 최신 호환 버전을
...
최소 1 GiB 여유 공간을 요구합니다. `summary.txt`와 `summary.tsv`에는 각 라운드의
...
# 벤치 서버의 2 logical CPU를 양쪽에 동일하게 배정하는 공식 3-round 비교
sudo BENCH_PROFILE=smoke BENCH_ROUNDS=3 BENCH_WORKERS=2 \
  BENCH_CPUS=2 BENCH_MEMORY=1g \
