...
Generic `x86-64-v2` release는 Fat LTO 링크 뒤 LLVM BOLT 19 instrumentation으로
HTTP/1.1, HTTP/2, Range, streaming, static 경로를 학습하고 post-link code layout을
최적화합니다. BOLT 입력에서만 symbol과 relocation을 보존하며 최종 binary는 다시 strip합니다.
정적 링크된 tcmalloc과 AWS-LC 특수 assembly는 내부 주소 및 code layout 제약이 있으므로
BOLT가 생성하는 중복 symbol 식별자까지 제외 목록에 넣어 원본 code를 보존합니다.
`BOLT_ENABLED=false`로 Fat LTO-only baseline을 재현할 수 있고 Actions는 비교용
`ghcr.io/tae-ok-11/pingora:fat-baseline`을 함께 게시합니다. BOLT profile은 정확한 binary와
같은 build 안에서 생성되므로 다른 revision의 stale profile을 재사용하지 않습니다.
GitHub runner CPU와 Oracle CPU가 같다는 보장이 없으므로 `oracle-znver1` tag에는 BOLT를
적용하지 않고 Fat LTO만 유지합니다.
...
PINGORA_EXPECTED_TARGET_CPU=znver1 tests/docker_runtime.sh`로 수행해야 합니다. 이 tag는
`lscpu`에서 AMD Zen 계열과 AVX2/BMI2 지원을 확인한 호스트에서만 사용하고, 호환 여부가
불명확하면 반드시 `latest`를 사용하십시오.
