...
`aws-lc-rs 1.17.1`, `aws-lc-sys 0.42.0`, `rustls 0.23.42`를 lockfile로 고정합니다.
...
Oracle image는 배포 대상과 같은 벤치 서버에서 `RUST_TARGET_CPU=native`, Fat LTO,
Rust/LLVM instrumentation PGO 순서로 빌드합니다. GitHub Actions는 generic
`x86-64-v2` image만 게시하고 Oracle image의 PGO 경로는 게시 없이 빌드 검증합니다.
...
`org.opencontainers.image.rust.pgo=train` 및
`org.opencontainers.image.rust.pgo-train-target-cpu=native`로 적용 여부를 확인할
수 있습니다. generic `latest`는 CPU 호환성을 우선해 non-PGO `x86-64-v2`로
유지합니다.

선택적 `BOLT_MODE=train`은 PGO-use binary를 `--emit-relocs`와 심볼을 보존해 링크한
뒤 BOLT instrumentation으로 같은 H1/H2/TLS/tail workload를 다시 학습합니다. BOLT가
실제 대상 바이너리를 처리하지 못하거나 PGO-only 대비 무결성·p99·처리량이 나빠지면
게시 이미지로 선택하지 않습니다. BOLT 도구와 profile 원본은 builder에만 존재하며
runtime에는 도구가 포함되지 않습니다.
...
# AMD Zen 벤치 서버 전용 PGO+BOLT 후보. PGO-only 후보는 BOLT_MODE=off.
docker build --build-arg ALLOCATOR=tcmalloc \
  --build-arg TLS_PROVIDER=aws-lc \
  --build-arg PGO_MODE=train \
  --build-arg PGO_TRAIN_TARGET_CPU=native \
  --build-arg BOLT_MODE=train \
  --build-arg RUST_TARGET_CPU=native \
  --build-arg RUST_LTO=fat \
  -t pingora:oracle-native-pgo-bolt .
```
...
`RUST_TARGET_CPU`로 전달하십시오. GitHub Actions는 generic `latest`만 게시하며,
Oracle 전용 `ghcr.io/tae-ok-11/pingora:oracle-znver1`은 배포 CPU와 같은 벤치 서버에서
native PGO/BOLT 후보를 비교하고 runtime test를 통과한 결과만 게시합니다. 검증은
`PINGORA_TEST_IMAGE=ghcr.io/tae-ok-11/pingora:oracle-znver1
PINGORA_EXPECTED_TARGET_CPU=native PINGORA_EXPECTED_PGO=train
